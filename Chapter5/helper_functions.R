...
#######################################################################################################################
## Helper function - 3: generate_substring_positions() 
#######################################################################################################################
...
#######################################################################################################################
## Helper function - 3.x: generate_substring_positions_prob() 
#######################################################################################################################

generate_substring_positions_prob <- function(Protein, mc_prob = missed_cleavage_probability,
    ...
    ## Remove cleavage sites with some cleavage probability (Check)
    cleavage_sites <- cleavage_sites[sample(c(FALSE,TRUE),
                                            size = length(cleavage_sites),
                                            replace = TRUE,
                                            prob = c(mc_prob,1-mc_prob))]
...
#######################################################################################################################
## Helper function - 4: generate_tryptic_peptides()
#######################################################################################################################
...
#######################################################################################################################
## Helper function - 4.x: generate_tryptic_peptides_prob() - Thies generates a single realisation of the digestion
#######################################################################################################################

generate_tryptic_peptides_prob <- function(Protein,
                                           max_missed_cleavages = 0,
                                           mc_prob = 0.1,
                                           remove_proline_sites = FALSE,
                                           methionine_peptides = TRUE){
    
    ## The missed cleavages are modelled outside this function so max_missed_cleavages is fixed at 0
    
    ## Initialize vector to store tryptic peptides (!!! I have truncated the vector lengths by 100x)
    Tryptic_Peptides <- character(length = 1000)
    
    MissedCleavages <- integer(length = 1000)
    
    start_position <- integer(length = 1000)
    end_position <- integer(length = 1000)
    ...
    ## vector of locations of starts and ends of substrings ("The Tryptic Peptides")
    substring_positions <- generate_substring_positions_prob(Protein = Protein,
                                                             mc_prob = mc_prob,
...
#######################################################################################################################
## Main function: generate_peptide_database()
#######################################################################################################################
