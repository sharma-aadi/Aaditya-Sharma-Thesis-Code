...
Compute_Log_Term <- function(eta_current, M_t){
    return(log(sum(dot(exp(eta_current), M_t))))
}
    ...
    for(i in 2:(iter)){
        ...
        for(s in 1:(Samples)){
            ...
            for(p in 1:(Proteins)){
                ...
                protein_prior_term <- dunif(eta_vector_new[p], min = 10, max = 25, log = TRUE) -
                    dunif(eta_vector_old[p], min = 10, max = 25, log = TRUE)    
